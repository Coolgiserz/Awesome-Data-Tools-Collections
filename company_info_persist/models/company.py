...
class Company(Base):
    ...
    registration_authority = Column(Text, comment='登记机关')
    paid_in_capital_value = Column(Float(53), comment='实缴资本（值）')
    paid_in_capital_unit = Column(String(32), comment='实缴资本（单位）')
    paid_in_capital_currency = Column(CHAR(32), comment='实缴资本（币种）')
    taxpayer_qualification = Column(Text, comment='纳税人资质')
